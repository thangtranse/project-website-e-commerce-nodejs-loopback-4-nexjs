...
import DNVNews from '../../components/Product/DNVNews'
...
const NewsView = ({
    ...
    useEffect(() => {
        try {
            ...
            console.log("thangtran.error.viewProduct.product:", error)
        }
    }, [location]);
                    ...
                    <Grid container spacing={1}>
                        {
                            product && product.length > 0 ? product.map((data, index) => {
                                return (
                                    <Grid item xs={12} sm={12} md={3} lg={3}>
                                        <div className={classes.animationBox}>
                                            <DNVNews
                                                image={`${process.env.REACT_APP_URL_FILE_IMAGE}${data.image}`}
                                                title={data.title}
                                                slug={pathDirectProduct + data.slug}
                                                description={ClearHTMLTag(data.description)}
                                ...
                                :
                                (
                                    <NotFoundElementView title={`${categoriesObject.charAt(0).toUpperCase()}${categoriesObject.slice(1, categoriesObject.length).toLowerCase()} not found`} />
                                )
                        }
                    </Grid>
                </InfiniteScroll>
